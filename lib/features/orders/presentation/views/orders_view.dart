...
import 'package:fruit_hub_dashboard/features/orders/presentation/manager/fetch_order/fetch_orders_cubit.dart';
import 'package:fruit_hub_dashboard/features/orders/presentation/manager/fetch_order/fetch_orders_state.dart';
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider<FetchOrdersCubit>(
          create: (context) =>
              FetchOrdersCubit(getIt.get<FetchOrdersUseCase>()),
        ),
        BlocProvider<UpdateOrderCubit>(
          create: (context) =>
              UpdateOrderCubit(getIt.get<UpdateOrderUseCase>()),
        ),
      ],
      child: Scaffold(
        appBar: AppBar(
    ...
    return BlocBuilder<FetchOrdersCubit, FetchOrdersState>(
      builder: (context, state) {
        if (state is FetchOrdersSuccess) {
          final newestFirstOrders =
              state.orders.reversed.toList(growable: false);
